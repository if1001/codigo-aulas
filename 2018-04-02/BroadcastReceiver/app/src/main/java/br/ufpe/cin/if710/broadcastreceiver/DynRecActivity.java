...
public class DynRecActivity extends Activity {
    public static final String DYN_BROADCAST_ACTION = "br.ufpe.cin.residencia.broadcasts.dinamico";
    private final IntentFilter intentFilter = new IntentFilter(DYN_BROADCAST_ACTION);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (podeSMS()) {
            Button enviarBroadcastSta = findViewById(R.id.enviarBroadcastSta);
            enviarBroadcastSta.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    sendBroadcast(new Intent(MainActivity.STA_BROADCAST_ACTION));
                }
            });
            Button enviarBroadcastDyn = findViewById(R.id.enviarBroadcastDyn);
            enviarBroadcastDyn.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    sendBroadcast(new Intent(DYN_BROADCAST_ACTION));
                }
            });
        }
        else {
    ...
    BroadcastReceiver smsReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            for (Object raw : rawMsgs) {
                ...
                if (msg.getMessageBody().toUpperCase().contains("IF1001")) {
